		...
		case PROTOCOL_REGISTER_C:
            {
				...
				MySqlQuery *query=static_cast<MySqlQuery *> (m_db_conn->createDbQuery());
				sprintf(buff,"select count(*) as num from snsuserinfo where PassportID=%s",PassportID);
                query->setSql(buff);
                MySqlDataSet *res=static_cast<MySqlDataSet *>(query->query());
				if(!res->isEmpty() && res->next())
				{
					...
					sprintf(buff,"insert into snsuserinfo (PassportID,PassportPwd) values ('%s','%s');  ",PassportID,PassporPwd);
					query->setSql(buff);
		...
		case PROTOCOL_LOGIN_C:
            {
                ...
                string PassporPwd = arrayObj["PassporPwd"].asString();
				MySqlQuery *query=static_cast<MySqlQuery *> (m_db_conn->createDbQuery());
				sprintf(buff,"select AID,PassportPwd from snsuserinfo where PassportID='%s'",PassportID);
                query->setSql(buff);
                MySqlDataSet *res=static_cast<MySqlDataSet *>(query->query());
				if(!res->isEmpty() && res->next())
				{
					MySqlField* pwd = static_cast<MySqlField *> (res->getFields("PassporPwd"));
					if(strcmp(PassporPwd.c_str(),pwd->asString())==0)
					{
						err=ERR_SUCCESS;
					}
					else
		...
		case PROTOCOL_SERVER_ADDR_C:
			{
				...
				rNo = PROTOCOL_SERVER_ADDR_S;
				string id = arrayObj["ID"].asString();
